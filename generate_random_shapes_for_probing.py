...
def gen_basic_problems(ntasks, save_dir='', process_id=0, max_num_all_tasks=5000,
    ...
    for i in range(max_num_all_tasks):
        num_lines = np.random.randint(1, 7)
        ...
        if probe_lengths and probe_angles:
            for _ in range(40):
                actions = []
                e = np.random.uniform(0,0.4,num_lines)
                a = np.random.uniform(0,5,num_lines)
                e.sort()
                idx=0
                for __ in range(num_lines):
                    l = lens[idx] + e[num_lines - idx - 1]
                    lt = 'normal'
                    td = 'L'
                    ta = angles[idx] + a[num_lines - idx - 1]
                    la = LineAction(line_length=l, line_type=lt, turn_direction=td, turn_angle=ta)
                    actions.append(la)
                    idx+=1 
            ...
            for _ in range(20):
                actions = []
                e = np.random.uniform(0,5,num_lines)
                e.sort()
                idx=0
                for __ in range(num_lines):
                    l = lens[idx]
                    lt = 'normal'
                    td = 'L'
                    ta = angles[idx] + e[num_lines - idx - 1]
                    la = LineAction(line_length=l, line_type=lt, turn_direction=td, turn_angle=ta)
                    actions.append(la)
                    idx+=1 
        ...
        basic_problem_name = str(i)+"_"+str(num_lines)

        bongard_problem = BongardProblem(positive_bongard_images=pos_shapes, negative_bongard_images=neg_shapes, problem_name=basic_problem_name)
        bongard_problem_painter = BongardProblemPainter(scaling_factors_range=(100,100), random_seed=121)
        # `BongardProblemPainter` will generate `ps` and `png` format images for the `BongardProblem` provided.
        bongard_problem_painter.create_bongard_problem(bongard_problem=bongard_problem,
                                                       bongard_problem_ps_dir=os.path.join(save_basic_dir, 'ps', basic_problem_name),
                                                       bongard_problem_png_dir=os.path.join(save_basic_dir, 'png', basic_problem_name))
        action_programs[basic_problem_name] = action
...
if __name__ == "__main__":
    ...
    parser.add_argument("--probe_angle",type=int, default=0, help="whether to probe angle")
    parser.add_argument("--probe_length", type=int, default=0, help="whether to probe length")
